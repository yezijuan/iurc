//后台请求地址
    	...
    	function remove(){
			...
			$.messager.confirm("提示消息","您确定要删除这"+array.length+"条记录？",function(r){
				...
				if(r){
					...
					$.ajax({
						url:"cooperation/deleteMore?ids="+ids,
						type:"post",
						dataType:"json",
    	...
    	function detail(){
			...
			//把选中的记录对象加载到form表单中填充
			$("#df").form("load",array[0]);
    	}
    	
		$(function(){
			$("#udg").datagrid({
				url:"cooperation/queryByPage",
				method:"post",
				columns:[[
				    {field:"",checkbox:true},
		          	{field:"id",title:"编号",width:50,align:"center",sortable:true,order:"desc"},
		          	{field:"cUnitId",title:"单位编号",width:50,align:"center"},
		          	{field:"projectId",title:"项目编号",width:200,align:"center"},
		          	{field:"contract",title:"项目内容",width:300,align:"center"},
		          	{field:"cooperatedTime",title:"创建时间",width:200,align:"center"},
